...
interface EmbedModelSelectorProps {
  value: string;
  onChange: (model: string) => void;
  apiKey: string;
}

export function EmbedModelSelector({ value, onChange, apiKey }: EmbedModelSelectorProps) {
  ...
  useEffect(() => {
    const fetchModels = async () => {
      if (!apiKey) {
        console.log('No API key provided to EmbedModelSelector');
        return;
      }
      ...
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/models/embed/models`, {
          method: 'POST',
          body: formData
        })
  ...
  useEffect(() => {
    console.log('EmbedModelSelector state:', {
      hasModels: models.length > 0,
      selectedModel,
      isLoading,
      apiKey: !!apiKey
    });
  }, [models, selectedModel, isLoading, apiKey]);
